...
execute if entity @e[tag=level1_gen,tag=!generated1] unless entity @e[tag=level1_gen_selected] as @e[tag=level1_gen,limit=1,tag=!generated1,sort=nearest] run tag @s add level1_gen_selected
execute if entity @e[tag=level1_gen_selected] as @e[tag=level1_gen_selected] run tag @s add generating1
...
execute if score @r level1_-Zcheck matches 2 run data modify entity @e[tag=level1_gen_selected,limit=1] ArmorItems[0].tag.StructureList.Array[15] set value 0
# Every Dead End and Intersection is deleted unless it is absolutely neccessary
# North/ -Z
...
# (Bug thoughts) RNG constantly running means that if for some reason this is running every tick with it on the same armor stand it could cycle through a few structures
...
execute at @r run data get entity @e[tag=level1_gen,limit=1,sort=nearest] Pos
execute at @r run data get entity @e[tag=level1_gen,limit=1,sort=nearest] Tags
...
execute if entity @e[tag=generating1] as @e[tag=generating1] run tag @s remove generating1
