...
execute if entity @a[scores={enter=1}] as @a[scores={enter=1}] at @s if dimension liminal:nowhere run fill ~ ~5 ~ ~ ~5 ~ yellow_concrete
execute if entity @a[scores={enter=1}] as @a[scores={enter=1}] at @s if dimension liminal:nowhere if block ~ ~5 ~ yellow_concrete run scoreboard players set @s enterTest 1
