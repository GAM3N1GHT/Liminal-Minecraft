...
execute if entity @e[tag=selected_l1,tag=generated1,tag=!generating1] as @a if score @s SelectedUUID = @s UUID if score @s level1_cangen matches 1 if score @s level1_totaldif <= @s distanceCheck run tag @e[tag=selected_l1] add new1
# If there is an armor stand that hasn't already generated something it makes it do so
execute if entity @e[tag=level1_gen,tag=!generated1,tag=!generating1] run function liminal:level1_genstuff
